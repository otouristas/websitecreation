...
type Variant = 'buttons' | 'inline' | 'icons';
...
/** WhatsApp-only contact CTA. */
export function ContactChannels({ variant = 'buttons', locale = 'en', className = '' }: ContactChannelsProps) {
  const isEl = locale === 'el';
  const label = isEl ? 'Μιλήστε μας στο WhatsApp' : 'Chat on WhatsApp';
  ...
  if (variant === 'inline') {
    return (
      <a
        ...
        rel="noopener noreferrer"
        onClick={() => trackCtaClick('contact_whatsapp')}
        className={`inline-flex items-center gap-1.5 text-sm font-medium text-foreground transition hover:text-primary ${className}`}
      >
        <WhatsAppIcon />
