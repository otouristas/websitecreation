/**
 * Greek translations for industry slugs (programmatic SEO).
 */
export const industriesEl: Record<
  string,
  {
    name: string;
    /**
     * Accusative form, used after "για" in titles and H1s. Greek inflects, so the
     * nominative reads broken in running copy: "για Δικηγόροι" -> "για Δικηγόρους".
     * Neuter plurals (Ξενοδοχεία, Ακίνητα) are identical in both cases.
     */
    nameAccusative: string;
    description: string;
    metaDescription: string;
    painPoints: string[];
  }
> = {
  restaurants: {
    name: 'Εστιατόρια',
    nameAccusative: 'Εστιατόρια',
    description:
      'Ιστοσελίδες για εστιατόρια, καφέ και επιχειρήσεις εστίασης. Online μενού, κρατήσεις τραπεζιών και τοπικό SEO.',
    metaDescription:
      'Επαγγελματικές ιστοσελίδες για εστιατόρια. Online μενού, κρατήσεις και βελτιστοποίηση τοπικής αναζήτησης.',
    painPoints: ['Online μενού', 'Κρατήσεις τραπεζιών', 'Τοπική ορατότητα', 'Παραγγελίες από κινητό'],
  },
  plumbers: {
    name: 'Υδραυλικοί',
    nameAccusative: 'Υδραυλικούς',
    description:
      'Ιστοσελίδες υψηλής μετατροπής για υδραυλικούς. Κράτηση επείγοντος, επιβεβαίωση περιοχής εξυπηρέτησης και σήματα εμπιστοσύνης.',
    metaDescription:
      'Ιστοσελίδες υδραυλικών που δημιουργούν επείγοντα τηλέφωνα. SEO για τοπικές υδραυλικές υπηρεσίες.',
    painPoints: ['Επείγοντα τηλέφωνα', 'Περιοχή εξυπηρέτησης', 'Επαλήθευση εμπιστοσύνης', 'Προγραμματισμός εργασιών'],
  },
  hvac: {
    name: 'Κλιματισμός & Θέρμανση',
    nameAccusative: 'Κλιματισμό & Θέρμανση',
    description:
      'Επαγγελματικές ιστοσελίδες για εταιρείες HVAC. Εποχιακές προσφορές, συμβόλαια συντήρησης και παρουσίαση εξοπλισμού.',
    metaDescription:
      'Ιστοσελίδες τεχνικών κλιματισμού με εποχιακές κρατήσεις και συμβόλαια συντήρησης.',
    painPoints: ['Εποχιακή ζήτηση', 'Προσφορές συστημάτων', 'Συμβόλαια συντήρησης', 'Επείγουσες επισκευές'],
  },
  electricians: {
    name: 'Ηλεκτρολόγοι',
    nameAccusative: 'Ηλεκτρολόγους',
    description:
      'Ιστοσελίδες που χτίζουν εμπιστοσύνη για αδειοδοτημένους ηλεκτρολόγους. Γκαλερί έργων, πιστοποιήσεις ασφαλείας και φόρμες προσφορών.',
    metaDescription:
      'Ιστοσελίδες ηλεκτρολόγων που χτίζουν εμπιστοσύνη. Παρουσίαση έργων, πιστοποιήσεις και περισσότερες προσφορές.',
    painPoints: ['Διασφάλιση ασφαλείας', 'Προσφορές έργων', 'Επείγουσα εξυπηρέτηση', 'Απόδειξη αδειοδότησης'],
  },
  // "Στεγαστικοί" reads as housing/mortgage in Greek (στεγαστικό δάνειο), not as
  // a roofing trade. The trade term is κατασκευαστές στεγών.
  roofers: {
    name: 'Κατασκευαστές Στεγών',
    nameAccusative: 'Κατασκευαστές Στεγών',
    description:
      'Ιστοσελίδες για κατασκευαστές στεγών και τεχνίτες στέγης, με στόχο leads υψηλής αξίας. Γκαλερί έργων, παρουσίαση υλικών και landing pages για επισκευή στέγης μετά από κακοκαιρία.',
    metaDescription:
      'Ιστοσελίδες για συνεργεία κατασκευής και επισκευής στέγης, βελτιστοποιημένες για leads υψηλής αξίας. Εργασίες στέγης και αντικαταστάσεις.',
    painPoints: ['Εμπιστοσύνη υψηλής αξίας', 'Leads επισκευής στέγης', 'Επιλογή υλικών', 'Ασφαλιστικές αποζημιώσεις'],
  },
  landscapers: {
    name: 'Κηπουρική',
    nameAccusative: 'Κηπουρική',
    description:
      'Οπτικά πρώτες ιστοσελίδες για εταιρείες κηπουρικής. Γκαλερί portfolio, προγραμματισμός συντήρησης και εποχιακές υπηρεσίες.',
    metaDescription:
      'Ιστοσελίδες κηπουρικής με εντυπωσιακά portfolios. Συμβόλαια συντήρησης και κράτηση συμβουλευτικών σχεδιασμού.',
    painPoints: ['Οπτικό portfolio', 'Επαναλαμβανόμενα συμβόλαια', 'Εποχιακές υπηρεσίες', 'Συμβουλευτικές σχεδίασης'],
  },
  painters: {
    name: 'Βαφείς',
    nameAccusative: 'Βαφείς',
    description:
      'Ιστοσελίδες δημιουργίας leads για βαφείς. Οπτικοποίηση χρωμάτων, γκαλερί έργων και άμεσες εκτιμήσεις προσφορών.',
    metaDescription:
      'Ιστοσελίδες βαφέων με γκαλερί έργων και φόρμες αιτήματος προσφοράς.',
    painPoints: ['Αξιοπιστία χρωμάτων', 'Προσφορές έργων', 'Οπτική απόδειξη', 'Προγραμματισμός'],
  },
  carpenters: {
    name: 'Ξυλουργοί',
    nameAccusative: 'Ξυλουργούς',
    description:
      'Ιστοσελίδες ξυλουργικής και κατασκευών στα μέτρα σας. Παρουσίαση portfolio για ντουλάπια, σκελετό και φινίρισμα.',
    metaDescription:
      'Ιστοσελίδες ξυλουργικής. Παρουσίαση εξατομικευμένων έργων και τεχνογνωσίας.',
    painPoints: ['Παρουσίαση τεχνογνωσίας', 'Εξατομικευμένες δυνατότητες', 'Χρονοδιάγραμμα έργου', 'Ποιότητα υλικών'],
  },
  flooring: {
    name: 'Δάπεδα',
    nameAccusative: 'Δάπεδα',
    description:
      'Ιστοσελίδες τοποθέτησης δαπέδων. Κατάλογοι υλικών, οπτικοποίηση χώρων και υπολογιστές προσφορών εγκατάστασης.',
    metaDescription:
      'Ιστοσελίδες τοποθέτησης δαπέδων. Showrooms για ξύλο, πλακάκι και μοκέτα.',
    painPoints: ['Οπτικοποίηση υλικών', 'Εκτιμήσεις στο σπίτι', 'Ερωτήματα αντοχής', 'Χρόνος εγκατάστασης'],
  },
  'cleaning-services': {
    name: 'Καθαρισμοί',
    nameAccusative: 'Καθαρισμούς',
    description:
      'Ιστοσελίδες με focus στις κρατήσεις για οικιακούς και επαγγελματικούς καθαρισμούς. Επαναλαμβανόμενες υπηρεσίες και άμεση τιμολόγηση.',
    metaDescription:
      'Ιστοσελίδες υπηρεσιών καθαρισμού. Κρατήσεις για οικιακό και επαγγελματικό καθαρισμό.',
    painPoints: ['Εμπιστοσύνη & ασφάλεια', 'Επαναλαμβανόμενες κρατήσεις', 'Εξατομικευμένες λίστες ελέγχου', 'Διαφάνεια τιμών'],
  },
  movers: {
    name: 'Μετακομίσεις',
    nameAccusative: 'Μετακομίσεις',
    description:
      'Ιστοσελίδες μετακομιστικών με υπολογιστές προσφορών, εκτίμηση αποστάσεων και κριτικές που χτίζουν εμπιστοσύνη.',
    metaDescription:
      'Ιστοσελίδες μετακομιστικών με engines προσφορών. Lead gen για τοπικές και μακρινές μετακομίσεις.',
    painPoints: ['Ακρίβεια προσφοράς', 'Εμπιστοσύνη & ασφάλεια', 'Διαθεσιμότητα ημερομηνίας', 'Ασφαλιστική κάλυψη'],
  },
  locksmiths: {
    name: 'Κλειδαράδες',
    nameAccusative: 'Κλειδαράδες',
    description:
      'Ιστοσελίδες βελτιστοποιημένες για επείγοντα για κλειδαράδες. Click-to-call, δείκτες 24/7 διαθεσιμότητας και χάρτες περιοχής.',
    metaDescription:
      'Ιστοσελίδες κλειδαράδων βελτιστοποιημένες για επείγοντα τηλέφωνα. Υπηρεσία 24/7 και mobile dispatch.',
    painPoints: ['Άμεση απόκριση', 'Mobile dispatch', 'Διαθεσιμότητα 24/7', 'Διαφάνεια τιμών'],
  },
  'pest-control': {
    name: 'Απεντόμωση',
    nameAccusative: 'Απεντόμωση',
    description:
      'Ιστοσελίδες απεντόμωσης με οδηγούς αναγνώρισης εντόμων, πλάνα θεραπείας και κράτηση επείγουσας εξυπηρέτησης.',
    metaDescription:
      'Ιστοσελίδες απεντόμωσης με πλάνα θεραπείας και βιβλιοθήκη εντόμων. Επείγουσες υπηρεσίες εξολοθρέωσης.',
    painPoints: ['Επείγον', 'Ανησυχίες ασφαλείας', 'Επαναλαμβανόμενα πλάνα', 'Αναγνώριση εντόμων'],
  },
  lawyers: {
    name: 'Δικηγόροι',
    nameAccusative: 'Δικηγόρους',
    description:
      'Ιστοσελίδες δικηγορικών γραφείων που χτίζουν εμπιστοσύνη, παρουσιάζουν εξειδίκευση και δημιουργούν qualified leads μέσω SEO.',
    metaDescription:
      'Ιστοσελίδες δικηγορικών γραφείων που χτίζουν εμπιστοσύνη και δημιουργούν leads. Σελίδες πρακτικών τομέων και τοπικό SEO.',
    painPoints: ['Χτίσιμο εμπιστοσύνης', 'Πρακτικοί τομείς', 'Αποτελέσματα υποθέσεων', 'Προφίλ δικηγόρων'],
  },
  dentists: {
    name: 'Οδοντίατροι',
    nameAccusative: 'Οδοντιάτρους',
    description:
      'Ιστοσελίδες οδοντιατρείων με online ραντεβού, μαρτυρίες ασθενών και εκπαιδευτικό περιεχόμενο για υπηρεσίες.',
    metaDescription:
      'Ιστοσελίδες οδοντιατρείων. Online κράτηση ραντεβού, παρουσίαση αισθητικής οδοντιατρικής και τοπικό SEO.',
    painPoints: ['Μείωση φόβου', 'Πληροφορίες ασφάλισης', 'Επείγοντα ραντεβού', 'Αισθητικά αποτελέσματα'],
  },
  chiropractors: {
    name: 'Χειροπράκτες',
    nameAccusative: 'Χειροπράκτες',
    description:
      'Ιστοσελίδες χειροπρακτικής με focus στην εκπαίδευση για ανακούφιση πόνου, προσφορές νέων ασθενών και online προγραμματισμό.',
    metaDescription:
      'Ιστοσελίδες χειροπράκτες με προσφορές νέων ασθενών. Εκπαίδευση για ανακούφιση πόνου και κράτηση ραντεβού.',
    painPoints: ['Ανακούφιση πόνου', 'Εισαγωγή νέων ασθενών', 'Ασφάλιση/Κόστος', 'Εξήγηση θεραπείας'],
  },
  therapists: {
    name: 'Ψυχοθεραπευτές',
    nameAccusative: 'Ψυχοθεραπευτές',
    description:
      'Ιστοσελίδες ιδιωτικής πρακτικής για θεραπευτές. Ασφαλείς φόρμες επικοινωνίας, περιγραφές ειδικοτήτων και οδηγοί «καλής αντιστοίχισης».',
    metaDescription:
      'Ιστοσελίδες ψυχοθεραπείας και συμβουλευτικής. Marketing ιδιωτικής πρακτικής και σύνδεση με ασθενείς.',
    painPoints: ['Απόρρητο/Εμπιστοσύνη', 'Αντιστοίχιση ειδικότητας', 'Κάλυψη ασφάλισης', 'Προσιτότητα'],
  },
  'real-estate': {
    name: 'Ακίνητα',
    nameAccusative: 'Ακίνητα',
    description:
      'Ιστοσελίδες ακινήτων με καταχωρήσεις, λειτουργικότητα αναζήτησης και στρατηγικές τοπικού SEO για την αγορά.',
    metaDescription:
      'Ιστοσελίδες ακινήτων με καταχωρήσεις, IDX integration και τοπικό SEO.',
    painPoints: ['Καταχωρήσεις ακινήτων', 'Δημιουργία leads', 'Ορατότητα στην αγορά', 'Εικονικές περιηγήσεις'],
  },
  hotels: {
    name: 'Ξενοδοχεία',
    nameAccusative: 'Ξενοδοχεία',
    description:
      'Ιστοσελίδες ξενοδοχείων και φιλοξενίας με ενσωμάτωση κρατήσεων, εντυπωσιακές γκαλερί δωματίων και SEO εστιασμένο στον τουρισμό - για ξενοδοχεία σε νησιά, πόλεις και παραθαλάσσιους προορισμούς που θέλουν άμεσες κρατήσεις αντί για υψηλές προμήθειες OTA.',
    metaDescription:
      'Κατασκευή ιστοσελίδας ξενοδοχείου με σύστημα κρατήσεων, galleries δωματίων και SEO φιλοξενίας για περισσότερες άμεσες κρατήσεις.',
    painPoints: [
      'Άμεσες κρατήσεις χωρίς OTA',
      'Παρουσίαση δωματίων & εμπειρίας',
      'Ανταγωνισμός Booking/Expedia',
      'Κριτικές & εμπιστοσύνη επισκεπτών',
    ],
  },
  gyms: {
    name: 'Γυμναστήρια',
    nameAccusative: 'Γυμναστήρια',
    description:
      'Ιστοσελίδες γυμναστηρίων με πρόγραμμα μαθημάτων, πληροφορίες συνδρομών και τοπικό SEO για fitness.',
    metaDescription:
      'Ιστοσελίδες γυμναστηρίων. Πρόγραμμα μαθημάτων, συνδρομές και τοπικό SEO.',
    painPoints: ['Πρόγραμμα μαθημάτων', 'Εγγραφές συνδρομών', 'Προφίλ προπονητών', 'Παρουσίαση εγκαταστάσεων'],
  },
  salons: {
    name: 'Κομμωτήρια & Spa',
    nameAccusative: 'Κομμωτήρια & Spa',
    description:
      'Ιστοσελίδες κομμωτηρίων και spa με μενού υπηρεσιών, συστήματα κράτησης και τοπικό SEO ομορφιάς.',
    metaDescription:
      'Ιστοσελίδες κομμωτηρίων και spa με online κράτηση, μενού υπηρεσιών και τοπικό SEO.',
    painPoints: ['Online κράτηση', 'Παρουσίαση υπηρεσιών', 'Προφίλ προσωπικού', 'Γκαλερί πριν/μετά'],
  },
  'plastic-surgeons': {
    name: 'Πλαστικοί Χειρουργοί',
    nameAccusative: 'Πλαστικούς Χειρουργούς',
    description:
      'Premium ιστοσελίδες για κλινικές πλαστικής χειρουργικής. Γκαλερί αποτελεσμάτων, οδηγοί επεμβάσεων και χτίσιμο εμπιστοσύνης ασθενών.',
    metaDescription:
      'Marketing και SEO για πλαστική χειρουργική. Απόκτηση ασθενών για ρινοπλαστική, αυξητική στήθους και άλλες επεμβάσεις.',
    painPoints: ['Εμπιστοσύνη ασθενών', 'Συμμόρφωση πριν/μετά', 'Premium branding', 'Ποιοτικά leads'],
  },
  'med-spas': {
    name: 'Med Spas',
    nameAccusative: 'Med Spas',
    description:
      'Κομψές, υψηλής μετατροπής ιστοσελίδες για Medical Spas. Ενσωμάτωση κρατήσεων, μενού θεραπειών και οπτική αφήγηση.',
    metaDescription:
      'Marketing και web design για Med Spa. SEO για Botox, fillers και αισθητικές θεραπείες.',
    painPoints: ['Ποσοστό κρατήσεων', 'Οπτική αίγλη', 'Εκπαίδευση θεραπειών', 'Επαναλαμβανόμενοι πελάτες'],
  },
  'dui-lawyers': {
    name: 'Δικηγόροι DUI',
    nameAccusative: 'Δικηγόρους DUI',
    description:
      'Υψηλής επείγοντος δημιουργία leads για δικηγόρους DUI. Click-to-call, χτίσιμο αυθεντίας και τοπική κυριαρχία.',
    metaDescription:
      'SEO και web design για δικηγόρους DUI. Επείγοντα leads άμυνας στην τοπική σας αγορά.',
    painPoints: ['Επείγον', 'Αυθεντία/Εμπιστοσύνη', 'Τοπική ορατότητα', 'Εμπειρία mobile'],
  },
  'personal-injury': {
    name: 'Προσωπικοί Τραυματισμοί',
    nameAccusative: 'Προσωπικούς Τραυματισμούς',
    description:
      'Επιθετικές στρατηγικές SEO για δικηγορικά γραφεία PI. Παρουσίαση αποτελεσμάτων, landing pages πρακτικών τομέων και lead capture.',
    metaDescription:
      'Marketing για δικηγόρους προσωπικών τραυματισμών. Δημιουργία υψηλής αξίας υποθέσεων για ατυχήματα και πτώσεις.',
    painPoints: ['Υψηλός ανταγωνισμός', 'Αποτελέσματα υποθέσεων', 'Σήματα εμπιστοσύνης', 'Όγκος leads'],
  },
  'financial-advisors': {
    name: 'Οικονομικοί Σύμβουλοι',
    nameAccusative: 'Οικονομικούς Συμβούλους',
    description:
      'Επαγγελματικές, συμβατές με κανονισμούς ιστοσελίδες για wealth managers. Χτίσιμο εμπιστοσύνης, resource hubs και κλείσιμο ραντεβού.',
    metaDescription:
      'SEO για οικονομικούς συμβούλους και wealth managers. Χτίστε εμπιστοσύνη και προσελκύστε πελάτες υψηλής καθαρής αξίας.',
    painPoints: ['Εμπιστοσύνη & αξιοπιστία', 'Συμμόρφωση', 'Ποιότητα leads', 'Παρουσίαση εξειδίκευσης'],
  },
  'rent-a-car': {
    name: 'Ενοικίαση Αυτοκινήτου',
    nameAccusative: 'Ενοικίαση Αυτοκινήτου',
    description:
      'Ιστοσελίδες rent-a-car με κατάλογο στόλου, funnels κρατήσεων, εποχιακές προσφορές και τοπικό SEO για νησιά, αεροδρόμια και τουριστικούς προορισμούς - ιδανικές για επιχειρήσεις ενοικίασης στην Ελλάδα που θέλουν κρατήσεις απευθείας από Google.',
    metaDescription:
      'Κατασκευή ιστοσελίδας ενοικίασης αυτοκινήτου με σελίδες στόλου, online κρατήσεις και SEO rent-a-car για νησιά και αεροδρόμια.',
    painPoints: [
      'Online κρατήσεις απευθείας',
      'Παρουσίαση στόλου & κατηγοριών',
      'Εποχιακή ζήτηση τουρισμού',
      'Ανταγωνισμός OTA & aggregators',
    ],
  },
  'tour-operators': {
    name: 'Οργανωτές Εκδρομών',
    nameAccusative: 'Οργανωτές Εκδρομών',
    description:
      'Ιστοσελίδες ημερήσιων εκδρομών και excursions με κατάλογο πακέτων, πλούσιες γκαλερί και SEO clusters ανά προορισμό - για tour operators που πουλούν κρουαζιέρες, island hopping, wine tours και εμπειρίες σε Σαντορίνη, Μύκονο, Κρήτη και άλλους ελληνικούς προορισμούς.',
    metaDescription:
      'Ιστοσελίδες tour operators με κατάλογο εκδρομών, CTAs κράτησης και SEO τουρισμού ανά προορισμό παγκοσμίως.',
    painPoints: [
      'Κρατήσεις εκδρομών online',
      'Εποχιακά πακέτα & early bird',
      'Γκαλερί & video περιεχόμενο',
      'Πολύγλωσσο SEO προορισμών',
    ],
  },
  'villas-apartments': {
    name: 'Βίλες & Διαμερίσματα',
    nameAccusative: 'Βίλες & Διαμερίσματα',
    description:
      'Ιστοσελίδες vacation rentals και βίλας με immersive galleries, φόρμες άμεσης κράτησης και διεθνές SEO - για ιδιοκτήτες βίλας και διαμερισμάτων που θέλουν να μειώσουν τις προμήθειες Airbnb/Booking και να προσελκύσουν ξένους επισκέπτες.',
    metaDescription:
      'Ιστοσελίδες βίλας και ενοικιαζομένων διαμερισμάτων. Άμεσες κρατήσεις, luxury galleries και SEO vacation rentals.',
    painPoints: [
      'Άμεσες κρατήσεις χωρίς OTA',
      'Εντυπωσιακές φωτογραφίες & video',
      'Ανταγωνισμός Airbnb/Booking',
      'Διεθνείς επισκέπτες & πολύγλωσσο SEO',
    ],
  },
  'travel-agencies': {
    name: 'Ταξιδιωτικά Γραφεία',
    nameAccusative: 'Ταξιδιωτικά Γραφεία',
    description:
      'Ιστοσελίδες ταξιδιωτικών γραφείων με παρουσίαση πακέτων, θεματικοί κόμβοι ανά προορισμό και συλλογή αιτημάτων για εξατομικευμένα δρομολόγια - ιδανικές για γραφεία που πουλούν πακέτα διακοπών, κρουαζιέρες και οργανωμένα ταξίδια στην Ελλάδα και το εξωτερικό.',
    metaDescription:
      'Web design και SEO για ταξιδιωτικά γραφεία. Σελίδες πακέτων, περιεχόμενο προορισμών και πολύγλωσσα funnels leads.',
    painPoints: [
      'Πώληση πακέτων & προσφορών',
      'Συλλογή αιτημάτων για εξατομικευμένα ταξίδια',
      'Περιεχόμενο ανά προορισμό',
      'Εμπιστοσύνη & κριτικές πελατών',
    ],
  },
  'travel-ai-chatbots': {
    name: 'AI Chatbots Τουρισμού',
    nameAccusative: 'AI Chatbots Τουρισμού',
    description:
      'Υλοποιήσεις AI βοηθών για τον τουρισμό - πολύγλωσση υποστήριξη κρατήσεων, αυτοματοποίηση FAQ και ορατότητα σε AI αναζήτηση (AEO) - για ξενοδοχεία, rent-a-car και tour operators που θέλουν 24/7 εξυπηρέτηση χωρίς να χάνουν κρατήσεις.',
    metaDescription:
      'Ανάπτυξη travel AI chatbots. Πολύγλωσσοι βοηθοί για ξενοδοχεία, ενοικίαση αυτοκινήτου και οργανωτές εκδρομών.',
    painPoints: [
      'Υποστήριξη 24/7 χωρίς προσωπικό',
      'Πολύγλωσσοι επισκέπτες',
      'Αυτοματοποίηση κρατήσεων & FAQ',
      'Ορατότητα σε ChatGPT & AI search',
    ],
  },
};
